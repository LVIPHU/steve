...
<template>
@D:/STEVE/steve/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "D:/STEVE/steve/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```bash
CHECKPOINT=$(node "D:/STEVE/steve/.claude/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "D:/STEVE/steve/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @D:/STEVE/steve/.claude/get-shit-done/workflows/diagnose-issues.md
