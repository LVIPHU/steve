...
- **Global scope**: `D:/STEVE/steve/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/STEVE/steve/.claude/notes/`)
...
2. Glob `D:/STEVE/steve/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/STEVE/steve/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/STEVE/steve/.claude/notes/` — works in any directory
